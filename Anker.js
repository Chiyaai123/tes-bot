...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Chiya\n'
            + 'ORG:Owner CHIYA BOT;\n'
            + 'TEL;type=CELL;type=VOICE;waid=628981481399:+62 898-1481-399\n'
            + 'END:VCARD'

prefix = "!"
name = "~ CHIYA BOT
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN CHIYA BOT😁"
rmenu = "HAI TEMEN CHIYA BOT👋 JANGAN LUPA DONASI YAA:)"
limitt = 10
ban = []
...
async function starts() {
	...
	fs.existsSync('./Chiya.json') && client.loadAuthInfo('./Chiya.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Chiya.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["08981481399@s.whatsapp.net"] // ubah aja gapapa
			...
			const ownerNumber = ["628981481399@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
					...
					if (isUser) return reply('kamu sudah Menjadi Temen CHIYABOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				case 'banlist':
					client.updatePresence(from, Presence.composing) 
					if (!isUser) return reply(mess.only.userB)
					ben = `╭─「 *TOTAL BANNED ${name}* 」\n`
					no = 0
					for (let benned of ban) {
						no += 1
					ben += `[${no.toString()}] @${benned.split('@')[0]}\n`
					}
					ben += `│+ Total User Kebanned : ${banned.length}\n╰──────⎿ *${name}* ⏋────`
...
//creator
			...
			reply(mess.wait)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					client.sendMessage(from, buff, image, {caption: 'Nih kak', quoted: mek})
					await limitAdd(sender) 
					break 
				case 'cparty':
					if (isBanned) return reply(mess.only.benned)    
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat CHIYABOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *CHIYABOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[CHIYABOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
